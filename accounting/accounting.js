...
(function(root, undefined) {
	...
	var formatNumber = lib.formatNumber = lib.format = function(params) {
		...
		if(!grouping) {
			return negative + base + (usePrecision ? opts.decimal + toFixed(Math.abs(number), usePrecision).split('.')[1] : "");
		}
		else {
			var firstParts = '';
			if(firstGrouping) {
				if(base.length > firstGrouping) {
					firstParts = opts.thousand + base.substring(base.length - firstGrouping, base.length);
					base = base.substring(0, base.length - firstGrouping);
				}
				else {
					return negative + base + (usePrecision ? opts.decimal + toFixed(Math.abs(number), usePrecision).split('.')[1] : "");
				}
